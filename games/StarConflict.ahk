        ...
        for _, x in range(401, A_ScreenWidth-401, 43) {
            for _, y in range(200, A_ScreenHeight-200, 43) {
                ...
                if (WinActive("ahk_exe game.exe") || WinActive("ahk_exe ACDSeeProfessional2018.exe"))
                {
                    MouseMove, x+100, y
                    ; Sleep 100
                    ImageSearch, FoundX, FoundY, oldX, oldY, x, y, *15 games/DOT1.jpg
                    if (ErrorLevel = 0)
                    {
...
sc_like() {
    if (WinActive("ahk_exe game.exe"))
    {
        ImageSearch, FoundX, FoundY, 0, 0, A_ScreenWidth, A_ScreenHeight, *80 games/players.jpg
        if (ErrorLevel = 0)
        {
            MouseClick, left, FoundX, FoundY,,,D
            Sleep 100
        ...
        sleep 100
        ImageSearch, FoundX, FoundY, 0, 0, A_ScreenWidth, A_ScreenHeight, *80 games/like.jpg
        if (ErrorLevel = 0)
        {
            MouseClick, left, FoundX, FoundY,,,D
            Sleep 100
